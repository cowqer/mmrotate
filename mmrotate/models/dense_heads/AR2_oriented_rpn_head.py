...
class AR2_OrientedRPNHead(RotatedRPNHead):
    """Oriented RPN head for Oriented R-CNN."""

    def _init_layers(self):
        """Initialize layers of the head.定义一个RPN卷积 分类head(卷积) 回归head(卷积)"""
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        Args:
            ...
            bbox_pred (torch.Tensor): Box energies / deltas for each scale
                level with shape (N, num_anchors * 5, H, W).5包括 4 个边界框坐标 (x、y、w、h) 和 1 个角度坐标（在旋转边界框的情况下）。
        ...
        if self.reg_decoded_bbox:
            ...
            bbox_pred = self.bbox_coder.decode(anchors, bbox_pred)# Apply transformation `pred_bboxes` to `bboxes`.
        ...
        loss_bbox = loss_bbox * aspect_ratio_weight
    ...
    def _get_bboxes_single(self,
        ...
        负责将原始模型输出转换为批次中单个图像的最终边界框预测。
        该方法处理多个特征图级别（尺度）的预测，并将它们组合成一组统一的边界框。
        """
        ...
        proposals = self.bbox_coder.decode( # Apply transformation `pred_bboxes` to `bboxes`.
