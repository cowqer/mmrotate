...
class DETRBaseTransformer(nn.Module):
    ...
    @staticmethod
    def flatten_multi_level(multi_level_elements):#多层特征（如多尺度特征图）拉平
    ...
    def multi_level_misc(self, multi_level_masks):#处理多层特征图的空间形状和起始索引，并计算有效区域的比例
    ...
    @staticmethod
    def get_valid_ratios(mask):#计算特征图中有效区域的比例
        b, h, w = mask.shape
    ...
    @staticmethod
    def get_full_reference_points(spatial_shapes, valid_ratios):#生成参考点，用于Transformer中的注意力计算
    ...
    def get_reference(self, spatial_shapes, valid_ratios):#生成全局参考点和候选框提案
