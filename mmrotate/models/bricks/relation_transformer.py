...
from models.bricks.ms_linear_attn import LiteMLA
...
class RelationTransformerEncoder(nn.Module):#整个编码器的封装，它包含多个编码层，同时，编码器引入了一个memory_fusion 模块，将每层的输出特征进行融合，最终生成一个更紧凑的编码结果。
...
class RelationTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.linear_attn = LiteMLA(in_channels=64, out_channels=64, scales=(5,))
...
class RelationTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        pos_relation = attn_mask  # fallback pos_relation to attn_mask在第0层之前把attnmask赋值给当前还没有的position里
        for layer_idx, layer in enumerate(self.layers):
            ...
            if not skip_relation:
                ...
                pos_relation = self.position_relation_embedding(src_boxes, tgt_boxes).flatten(0, 1)#取上一层和本层的作为输入
                if attn_mask is not None:
                    pos_relation.masked_fill_(attn_mask, float("-inf"))#赋值负无穷
...
def box_rel_encoding(src_boxes, tgt_boxes, eps=1e-5):#位置关系嵌入
