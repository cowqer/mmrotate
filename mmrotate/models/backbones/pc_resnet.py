...
# from mmrotate.models.backbones.base_backbone import BaseBackbone
# from mmrotate.models.bricks.deform_conv2d_pack import DeformConv2dPack
# from mmrotate.models.bricks.misc import FrozenBatchNorm2d
# from mmrotate.utils.util.lazy_load import LazyCall as L
# from mmrotate.utils.util.lazy_load import instantiate
# from mmrotate.utils.util.tune_mode_convbn import \
#     turn_on_efficient_conv_bn_eval_for_single_model
# from mmrotate.utils.util.utils import load_checkpoint

# from mmrotate.models.builder import ROTATED_BACKBONES
# from ..utils import PConv
...
# # 构建一个基本卷积（残差）块 用于Resnet-18/-34
# class BasicBlock(nn.Module):
#     expansion: int = 1 # 扩展因子（通常为4）用于增加输出通道的数量。
...
#         if self.downsample is not None:# 在上面设置了步长为一 如果存在下采样层
...
# class Bottleneck(nn.Module):# 用于ResNet-50、ResNet-101、ResNet-152
...
#     expansion: int = 4 # 扩展因子（通常为4）用于增加输出通道的数量。
...
#         self.conv2 = PConv( # normal convolution change to pconv
#                 c1=planes,
#                 c2=planes,
#                 k=3,
#                 s=self.conv2_stride
...
#         x = self.layer1(x) # self.layer1 = self._make_layer(block, 64, layers[0], with_dcn=stage_with_dcn[0])
#         x = self.layer2(x)
#         x = self.layer3(x)
#         x = self.layer4(x)
...
#         super(PCResNet, self).train(mode)
#         for m in self.modules():
...
#     model_arch = {#创建相应的网络结构实例
#         "resnet18": L(PCResNet)(block=BasicBlock, layers=(2, 2, 2, 2), url=model_weights["resnet18"]),
#         "resnet34": L(PCResNet)(block=BasicBlock, layers=(3, 4, 6, 3), url=model_weights["resnet34"]),
#         "resnet50": L(PCResNet)(block=Bottleneck, layers=(3, 4, 6, 3), url=model_weights["resnet50_v2"]),
#         "resnet101": L(PCResNet)(block=Bottleneck, layers=(3, 4, 23, 3), url=model_weights["resnet101_v2"]), 
#         "resnet152": L(PCResNet)(block=Bottleneck, layers=(3, 8, 36, 3), url=model_weights["resnet152_v2"]),
#         "resnext50_32x4d": L(PCResNet)(
#             block=Bottleneck,
#             layers=(3, 4, 6, 3),
#             groups=32,
#             width_per_group=4,
...
#         ),
#         "wide_resnet50_2": L(PCResNet)(
#             block=Bottleneck,
#             layers=(3, 4, 6, 3),
#             width_per_group=64 * 2,
#             url=model_weights["wide_resnet50_2_v2"],
#         ),
#         "wide_resnet101_2": L(PCResNet)(
#             block=Bottleneck,
#             layers=(3, 4, 23, 3),
#             width_per_group=64 * 2,
...
#         model_config = self.get_instantiate_config(self, PCResNet, arch, kwargs)
