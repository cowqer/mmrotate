...
class RountingFunction_SCSA(nn.Module):

    def __init__(self, in_channels, kernel_number, dropout_rate=0.2, proportion=40.0):
        super().__init__()
        self.kernel_number = kernel_number
        self.pc = PConv(in_channels, in_channels, k=3, s=1)
        ...
        # 实例化 SCSA
        self.scsa = SCSA(dim=in_channels, window_size=7, group_kernel_sizes=[3, 5, 7, 9], qkv_bias=False, fuse_bn=False, down_sample_mode='avg_pool', attn_drop_ratio=0.0, gate_layer='sigmoid')
...
if __name__ == "__main__":
    # 创建一个 RountingFunction_SCSA 实例
    in_channels = 64
    kernel_number = 16
    dropout_rate = 0.2
    proportion = 40.0

    rounting_function_scsa = RountingFunction_SCSA(in_channels, kernel_number, dropout_rate, proportion)
    ...
    # 前向传播
    alphas, angles = rounting_function_scsa(input_tensor)
