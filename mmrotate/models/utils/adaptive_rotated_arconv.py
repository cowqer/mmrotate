...
def batch_rotate_multiweight(weights, lambdas, thetas, ar):
    """
    Let
        batch_size = b
        kernel_number = n
    ...
    ar = ar.unsqueeze(2).unsqueeze(3)
    rotation_matrix = torch.mul(rotation_matrix, lambdas)##eq3 in the paper——逐点相乘
    rotation_matrix = torch.mul(rotation_matrix, ar)
...
class AdaptiveRotatedConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        ar, alphas, angles = self.rounting_func(x)
